...
export default function EditDocumentTypeName() {
  ...
  const addMutation = useMutation({
    mutationFn: (newEquipment) => axiosInstance.post('/document-types', newEquipment),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...updatedData }) =>
      axiosInstance.put(`/document-types/${id}`, updatedData),
    onSuccess: () => {
  ...
  const columns = [
    { field: 'name', headerName: 'Document Type Name', width: 250 },
  ...
  return (
    <OPPageContainer sx={{ px: 4, pt: 2 }}>
      ...
      >
        <Typography variant="h5">{selectedRow ? 'Edit Document Type' : 'Add New Document Type'}</Typography>
        <Divider />
        <form
