...
export default function EditEquipmentName() {
  ...
  const addMutation = useMutation({
    mutationFn: (newEquipment) => axiosInstance.post('/equipments', newEquipment),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, ...updatedData }) =>
      axiosInstance.put(`/equipments/${id}`, updatedData),
    onSuccess: () => {
  ...
  const columns = [
    { field: 'name', headerName: 'Equipment Name', width: 250 },
  ...
  return (
    <OPPageContainer sx={{ px: 4, pt: 2 }}>
      ...
      >
        <Typography variant="h5">{selectedRow ? 'Edit Equipment' : 'Add New Equipment'}</Typography>
        <Divider />
        <form
